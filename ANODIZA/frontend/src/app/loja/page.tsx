...
type CategoriaMaterial = "portas" | "vidros" | "perfis" | "puxadores" | "sistemas" | "trilhos" | "insumos";
...
export default function LojaPage() {
  ...
  const [categoriaMaterial, setCategoriaMaterial] = useState<CategoriaMaterial>("portas");
  ...
  useEffect(() => {
    ...
    if (opcoesSalvas) {
      try { setOpcoesSistema({ ...opcoesPadrao, ...(JSON.parse(opcoesSalvas) as Partial<Record<OpcaoSistema, boolean>>) }); } catch { setOpcoesSistema(opcoesPadrao); }
    }
    setEmpresaSlug(slug);
  ...
  function handleLogoUpload(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = () => {
      ...
      setLogoMarca(value);
      localStorage.setItem("anodiza_logo", value);
      setMensagemLogo("Logo atualizada. Ela substituirá o A do menu.");
    };
  ...
  async function handleCriarCliente(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!empresaSlug) return setMensagemClientes("Entre novamente para identificar a empresa.");
    if (!novoCliente.nome.trim()) return setMensagemClientes("Informe o nome do cliente.");
    setMensagemClientes("Cadastrando cliente...");
    try { await apiPost("/api/loja/clientes", { empresa_slug: empresaSlug, ...novoCliente }); setNovoCliente(clienteVazio); setMensagemClientes("Cliente cadastrado."); await carregarClientes(empresaSlug); }
    catch (error) { setMensagemClientes(`Nao foi possivel cadastrar o cliente. ${error instanceof Error ? error.message : "Erro desconhecido"}`); }
  }

  async function handleEditarCliente(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!clienteEditando) return;
    if (!clienteEditando.nome.trim()) return setMensagemClientes("Informe o nome do cliente.");
    setMensagemClientes("Salvando cliente...");
    try { await apiPost("/api/loja/clientes/editar", { empresa_slug: empresaSlug, ...clienteEditando }); setClienteEditando(null); setMensagemClientes("Cliente atualizado."); await carregarClientes(empresaSlug); await carregarOrcamentos(empresaSlug, buscaOrcamento); }
    catch (error) { setMensagemClientes(`Nao foi possivel editar o cliente. ${error instanceof Error ? error.message : "Erro desconhecido"}`); }
  }
  ...
  async function handleCriarOrcamento(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!empresaSlug) return setMensagemNovoOrcamento("Entre novamente para identificar a empresa.");
    if (!novoOrcamento.nome_orcamento.trim()) return setMensagemNovoOrcamento("Informe o nome do orcamento.");
    if (!novoOrcamento.cliente_id) return setMensagemNovoOrcamento("Selecione o cliente.");
    setMensagemNovoOrcamento("Criando orcamento...");
    try { await apiPost("/api/loja/orcamentos", { empresa_slug: empresaSlug, ...novoOrcamento }); setNovoOrcamento((current) => ({ nome_orcamento: "", cliente_id: current.cliente_id })); setMensagemNovoOrcamento("Orcamento criado com numeracao automatica do cliente."); await carregarOrcamentos(empresaSlug, buscaOrcamento); }
    catch (error) { setMensagemNovoOrcamento(`Nao foi possivel criar o orcamento. ${error instanceof Error ? error.message : "Erro desconhecido"}`); }
  }
  ...
  async function handleCriarUsuario(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!empresaSlug) return setMensagem("Entre novamente para identificar a empresa.");
    setMensagem("Criando usuario...");
    try { await apiPost("/api/loja/usuarios", { empresa_slug: empresaSlug, ...novoUsuario }); setNovoUsuario({ nome: "", email: "", perfil: "vendedor" }); setMensagem("Usuario criado."); await carregarUsuarios(empresaSlug); }
    catch (error) { setMensagem(`Nao foi possivel criar o usuario. ${error instanceof Error ? error.message : "Erro desconhecido"}`); }
  }
