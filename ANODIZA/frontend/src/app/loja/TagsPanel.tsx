...
type TagForm = { id: string; nome: string; descricao: string; categorias_aplicaveis: CategoriaMaterial[]; ativo: boolean };
...
function dinheiro(valor: number) { return new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(valor || 0); }
function numero(valor: string) { const n = Number(String(valor || "").replace(",", ".")); return Number.isFinite(n) ? n : 0; }
function categoriaLabel(categoria: CategoriaMaterial) { return categorias.find((item) => item.value === categoria)?.label || categoria; }
function unidadeLabel(unidade: UnidadeCalculo) { return unidades.find((item) => item.value === unidade)?.label || unidade; }
function listaToggle<T extends string>(lista: T[], valor: T) { return lista.includes(valor) ? lista.filter((item) => item !== valor) : [...lista, valor]; }
...
export default function TagsPanel({ empresaSlug }: { empresaSlug: string }) {
  ...
  function tagsPorCategoria(categoria: CategoriaMaterial) { return tags.filter((tag) => tag.ativo && ((tag.categorias_aplicaveis || []).length === 0 || (tag.categorias_aplicaveis || []).includes(categoria))); }
  function nomesTags(ids?: string[]) { const nomes = (ids || []).map((id) => tagPorId.get(id)?.nome || id).filter(Boolean); return nomes.length ? nomes.join(", ") : "Qualquer característica"; }
  function materiaisComTags(material: Material) { return nomesTags(materialTags.filter((item) => item.material_id === material.id).map((item) => item.tag_id)).replace("Qualquer característica", "Sem tags"); }

  async function salvarTag(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setMensagem("");
    try {
      if (tagForm.id) await apiPost("/api/loja/tags/editar", { empresa_slug: empresaSlug, ...tagForm });
      else await apiPost("/api/loja/tags", { empresa_slug: empresaSlug, ...tagForm });
      setTagForm(tagInicial);
      await carregarTudo();
    } catch (error) { setMensagem(error instanceof Error ? error.message : "Erro ao salvar tag"); }
  }

  async function excluirTag(tag: Tag) {
    if (!window.confirm(`Excluir a tag ${tag.nome}?`)) return;
    try { await apiPost("/api/loja/tags/excluir", { empresa_slug: empresaSlug, id: tag.id }); await carregarTudo(); }
    catch (error) { setMensagem(error instanceof Error ? error.message : "Erro ao excluir tag"); }
  }

  async function salvarTagsMaterial() {
    if (!materialSelecionado) { setMensagem("Selecione um material"); return; }
    try { await apiPost("/api/loja/tags/material", { empresa_slug: empresaSlug, material_id: materialSelecionado, tag_ids: tagsSelecionadasMaterial }); await carregarTudo(); }
    catch (error) { setMensagem(error instanceof Error ? error.message : "Erro ao salvar tags do material"); }
  }
  ...
  async function salvarRegra(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await carregarTudo();
    } catch (error) { setMensagem(error instanceof Error ? error.message : "Erro ao salvar regra"); }
  }

  async function excluirRegra(regra: RegraTag) {
    if (!window.confirm(`Excluir a regra ${regra.nome}?`)) return;
    try { await apiPost("/api/loja/tags/regras/excluir", { empresa_slug: empresaSlug, id: regra.id }); await carregarTudo(); }
    catch (error) { setMensagem(error instanceof Error ? error.message : "Erro ao excluir regra"); }
  }

  return (
    <section className="card" style={{ maxWidth: "none" }}>
      <h1>Tags e regras de negócio</h1>
      <p>Crie características para materiais e regras que cruzam dois grupos de materiais, incluindo Sistemas.</p>
